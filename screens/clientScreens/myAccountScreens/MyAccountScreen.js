...
const MyAccountScreen = ({ navigation }) => {
    ...
    const ButtonTitleComponent = ({ icon, text }) => (
        ...
        >
            ...
            <AntDesign name="caretright" size={32} color={color.black} />
        </View>
    )
   let profileButtonData = [
    ...
    {
        icon: <MaterialIcons name="exit-to-app" size={32} color={color.redstrong} />, 
        text: "Çıkış Yap",
        route:"login",
    },
   ]
        ...
        <ScreenWrapper>
            ...
            { profileButtonData.map(( field, index ) => (
                <ButtonOriginal 
                  key={index}
                  buttonStyle={{ 
                      marginTop: 10, flexDirection: "row", height: 60,
                      backgroundColor: "transparent", shadowOpacity: 0, 
                  }}
                  textStyle={{ flex: 1, color: color.black, justifyContent: "center" }}
